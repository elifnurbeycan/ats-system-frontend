...
export function EditPipelineDialog({ open, pipeline, saving, onOpenChange, onSave }: { open: boolean; pipeline: Pipeline | null; saving: boolean; onOpenChange: (open: boolean) => void; onSave: (payload: PipelineEditPayload) => void }) {
  ...
  return <Dialog open={open} onOpenChange={(value) => !saving && onOpenChange(value)}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle>İşe alım sürecini düzenle</DialogTitle><DialogDescription>İşe alım süreci bilgilerini ve aşamalarını güncelleyin. Kullanımda aday bulunan aşamalar silinemez.</DialogDescription></DialogHeader>
    <div className="space-y-4 py-2">
      <div className="space-y-1.5"><label className="text-sm font-medium">İşe alım süreci adı *</label><Input value={form.name} maxLength={150} onChange={(e) => setForm({ ...form, name: e.target.value })} /></div>
      ...
      <label className="flex items-center gap-2 rounded-md border border-border p-3 text-sm"><input type="checkbox" checked={form.defaultPipeline} onChange={(e) => setForm({ ...form, defaultPipeline: e.target.checked })} />Varsayılan işe alım süreci olarak ayarla</label>
      <div className="space-y-2">
        <div className="flex items-center justify-between gap-3"><div><p className="text-sm font-semibold">İşe alım süreci aşamaları</p><p className="text-xs text-muted-foreground">Silme işlemi kaydettiğinizde uygulanır.</p></div><Button type="button" variant="outline" size="sm" onClick={addStage}><Plus className="mr-1.5 h-4 w-4" />Yeni aşama ekle</Button></div>
        ...
        {!hasActiveStage && <p className="text-xs text-destructive">İşe alım süreci içinde en az bir aktif süreç aşaması bulunmalıdır.</p>}
      </div>
    </div>
