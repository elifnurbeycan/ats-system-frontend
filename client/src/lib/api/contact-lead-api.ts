...
export const contactLeadApi={
 ...
 getPage:async(options:{page?:number;size?:number;search?:string;status?:ContactLeadStatus;rejectionReason?:ContactRejectionReason}={}):Promise<PageData<ContactLead>>=>(await apiClient.get<ApiResponse<PageData<ContactLead>>>(`/${getCompanyId()}/contact-leads`,{params:options})).data.data,
